...
from utils.generic_utils import load_dataset_at, calculate_dataset_metrics, cutoff_choice, \
                                cutoff_sequence
from utils.new_constants import MAX_NB_VARIABLES, MAX_TIMESTEPS_LIST
...
def train_model(model:Model, dataset_id, dataset_fold_id=None, epochs=50, batch_size=128, val_subset=None,
                cutoff=None, normalize_timeseries=False, learning_rate=1e-3, monitor='loss', optimization_mode='auto',
                compile_model=True, filters1=None, filters2=None, nodes1=None, dropout1=None):
    
    # Cogemos X e y del dataset; le quitamos el sufijo de _train porque eso lo reservamos para hacerlo
    # dentro del KFold
    X, y, X_test, y_test, is_timeseries = load_dataset_at(dataset_id,
                                                                      fold_index=dataset_fold_id,
                                                                      normalize_timeseries=normalize_timeseries)
    max_timesteps, max_nb_variables = calculate_dataset_metrics(X)
    ...
    if max_nb_variables != MAX_NB_VARIABLES[dataset_id]:
        ...
        #     assert cutoff in ['pref', 'post'], 'Cutoff parameter value must be either "pre" or "post"'
        #     choice = cutoff
        ...
        # else:
            choice='post'
            X, X_test = cutoff_sequence(X, X_test, choice, dataset_id, max_nb_variables)

    classes = np.unique(y)
    le = LabelEncoder()
    y_ind = le.fit_transform(y.ravel())
    ...
    # if val_subset is not None: # Como sí que es None, comentamos esto
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(X)):
        ...
        #accuracy_results.append(eval_results[1])  # Append accuracy to the list

        file_cv = open("results_train_cv.txt", "a")  # append mode
        file_cv.write(str(dataset_id) +";" +str(fold)+";"+str(eval_results[1])+";"+str(filters1)+";"+str(filters2)+';'+str(nodes1)+";"+str(dropout1)+"\n")
        file_cv.close()
    # Quitado , callbacks=callback_list, class_weight=class_weight, validation_data=(X_test, y_test)


def train_selected_model(model:Model, dataset_id, dataset_fold_id=None, epochs=50, batch_size=128, val_subset=None,
...
def evaluate_model(model:Model, dataset_id, dataset_fold_id=None, batch_size=128, test_data_subset=None,
                   cutoff=None, normalize_timeseries=False, startTime=0):
    ...
    #file1 = open("results_antonio_seq2var_lstm.txt", "a")  # append mode
    file1 = open("results_cv.txt", "a")  # append mode
    file1.write(str(dataset_id) +";" +str(accuracy )+";"+str(executionTime)+str(0)+"\n")
    file1.close()
